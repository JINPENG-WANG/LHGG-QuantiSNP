	...
	while(<$fh_fr_in>){
		...
		if($line_count_fr>10){
			...
			if(exists $indsignals{$sampleid}){
				...
				$fh_out->print("$sampleid\t$snpid\t$chr\t$pos\t$lrr\t$baf\n");
			}
			else{
				my $fh_out = IO::File->new(">quantiSNPsignalfiles/$sampleid.signals.txt");
				$indsignals{$sampleid}=$fh_out;
				$indsignals{$sampleid}->print("Sample ID\tSNP Name\tChromosome\tPosition\tLog R Ratio\tB Allele Freq\n");
				$indsignals{$sampleid}->print("$sampleid\t$snpid\t$chr\t$pos\t$lrr\t$baf\n");
			}
		}
	}
}
